  ...
  render() {
    ...
    content.forEach(item => {
      ...
      el.innerHTML = `
        ...
        ${item.type === 'video' 
          ? `<video src="${item.data}" controls class="w-full rounded-xl" muted></video>` 
          : `<pre class="bg-black p-4 rounded-xl text-green-400 font-mono text-sm overflow-x-auto">${atob(item.data.split(',')[1])}</pre>`
        }
      `;
      feed.appendChild(el);
    });
